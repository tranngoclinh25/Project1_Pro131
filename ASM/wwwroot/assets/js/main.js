...
/*----------------------------------------*/
	...
	$(function () {
		...
		function formatCurrency(number) {
			return number.toLocaleString('vi-VN', { style: 'currency', currency: 'VND', minimumFractionDigits: 0 });
		}

		function getMaxPrice(productList) {
			let maxPrice = 0;
		...
		var maxPrice = getMaxPrice(productList);
		...
		sliderRange.slider({
			...
			slide: function (event, ui) {
				amountInput.val(formatCurrency(ui.values[0]) + " - " + formatCurrency(ui.values[1]));
			},
			change: function () {
				updateFilter();
			}
		...
		amountInput.val(formatCurrency(sliderRange.slider("values", 0)) + " - " + formatCurrency(sliderRange.slider("values", 1)));

		$('#filter-button').on('click', function () {
			updateFilter();
		});

		$('#refine-item').change(function () {
			updateFilter();
		});
		...
		function updateFilter() {
			...
			var onSaleChecked = $('#refine-item').is(':checked');
			var searchValue = document.getElementById('search-input').value.toLowerCase();

			$('.product-item').each(function () {
				var itemPrice = parseFloat($(this).find('.new-price').text().replace(/[^0-9.]/g, ''));
				var productName = $(this).find('.product-name').text().toLowerCase();

				var meetsPriceCondition = (itemPrice >= minPrice && itemPrice <= maxPrice);
				var meetsOnSaleCondition = ($(this).hasClass('on-sale') && onSaleChecked) || !onSaleChecked;
				var meetsSearchCondition = (productName.indexOf(searchValue) > -1);

				if (meetsPriceCondition && meetsOnSaleCondition && meetsSearchCondition) {
					$(this).show();
				} else {
					$(this).hide();
				}
		...
		document.getElementById("category-filter").addEventListener("change", function () {
			...
			var productListItems = productListContainer.getElementsByClassName("product-item");

			for (var i = 0; i < productListItems.length; i++) {
				var productItem = productListItems[i];
				var categoryId = productItem.getAttribute("data-category-id");
